#! Certificate renewer — untrusted a request can only ever become one of a fixed set of decisions over a
...
#! @requires renew — the certificate renewer sink
#! @effect io
#! @irreversible
#! @taint bridge — extract<Decision> turns the tainted input into a trusted decision
grant renew irreversible

type CertDomain = Apex | Wildcard | Sub
type Decision = Renew(CertDomain) | Reject
...
commit { renew(d) }  # act on the trusted decision only
