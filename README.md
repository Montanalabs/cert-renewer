...
- **Closed type:** `type Decision = Renew(CertDomain) | Reject` where `CertDomain` is closed
- **Sink / capability:** `grant renew irreversible` → `commit { renew(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/cert-renewer/demo.sh
```
...
- `cert-renewer_safe.wave` — the correct design.
- `cert-renewer_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
